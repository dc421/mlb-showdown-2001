...
const showAutoThrowResult = computed(() => {
  if (!gameStore.gameState?.throwRollResult  || (!!gameStore.gameState.inningEndedOnCaughtStealing && gameStore.amIReadyForNext)) {
    return false;
  }
...
const showStealResult = computed(() => {
  if (!stealResultDetails.value || (gameStore.gameState.inningEndedOnCaughtStealing.value && gameStore.amIReadyForNext.value)) {
    return false;
  }
