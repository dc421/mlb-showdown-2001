...
const myBenchAndBullpen = computed(() => {
    ...
    if (myPitcher) {
        if (useDh.value || myLineup.value.battingOrder.some(s => s.player?.card_id === myPitcher.card_id)) {
...
const homeBenchAndBullpen = computed(() => {
    ...
    if (homePitcher.value) {
        if (useDh.value || gameStore.lineups.home.battingOrder.some(s => s.player?.card_id === homePitcher.value.card_id)) {
            lineupIds.add(homePitcher.value.card_id);
        }
    }
    if (isDisplayTopInning.value && pitcherToDisplay.value) {
        if (useDh.value || gameStore.lineups.home.battingOrder.some(s => s.player?.card_id === pitcherToDisplay.value.card_id)) {
...
const awayBenchAndBullpen = computed(() => {
    ...
    if (awayPitcher.value) {
        if (useDh.value || gameStore.lineups.away.battingOrder.some(s => s.player?.card_id === awayPitcher.value.card_id)) {
            lineupIds.add(awayPitcher.value.card_id);
        }
    }
    if (isDisplayTopInning.value === false && pitcherToDisplay.value) {
        if (useDh.value || gameStore.lineups.away.battingOrder.some(s => s.player?.card_id === pitcherToDisplay.value.card_id)) {
            lineupIds.add(pitcherToDisplay.value.card_id);
        }
    }
...
const showRollForPitchButton = computed(() => {
  if (gameStore.isSubmittingPitch) return false;
...
const showSwingAwayButton = computed(() => {
  if (gameStore.isSubmittingAction) return false;
  if (isGameOver.value && isSwingResultVisible.value) return false;
  if (isGameOver.value && isGameEndingSteal.value) return false;
...
async function handleInitiateSteal(decisions) {
    isTransitioningToNextHitter.value = false;
    gameStore.setIsStealResultVisible(true);
    await gameStore.initiateSteal(gameId, decisions);
    

//inningEndedOnCaughtStealing true, amIReadyForNext true -> don't auto resolve (BEHIND)
//inningEndedOnCaughtStealing true, amIReadyForNext false -> auto resolve (EVEN)

    // Auto-resolve if both players are already on this at-bat and it's a single steal
    if (!gameStore.opponentReadyForNext && !gameStore.amIReadyForNext) {
        ...
        if (stealingRunners.length === 1) {
            const throwToBase = parseInt(stealingRunners[0], 10) + 1;
...
<div v-if="!showSetLineupForNextGameButton && !(isGameOver && isSwingResultVisible && !gameStore.gameState.pendingStealAttempt && !((gameStore.gameState?.doublePlayDetails && !showThrowRollResult)))" class="actions-container">
        ...
        <div v-if="isMyTeamAwaitingLineupChange" class="waiting-text">
            ...
            <div v-else-if="isStealAttemptInProgress && !isSingleSteal && amIDisplayDefensivePlayer && (!showStealResult || (!!gameStore.gameState?.lastStealResult && !!gameStore.gameState?.pendingStealAttempt))">
                <div>
